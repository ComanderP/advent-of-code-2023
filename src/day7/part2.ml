...
let compare_hands (h1 : hand) (h2 : hand) : int =
  let cmp =
    compare (int_of_hand_type h1.hand_type) (int_of_hand_type h2.hand_type)
  in
  if cmp <> 0 then cmp
  else
...
let rec contains_joker = function
  | [] -> false
  | h :: t -> if h = 1 then true else (contains_joker [@tailcall]) t
...
let string_to_hand (hand : string list) =
  ...
  if contains_joker hand then
    let hand_without_joker = List.filter (fun x -> x <> 1) hand in
    let counts = count_chars hand_without_joker in
    match List.length hand_without_joker with
    | 4 ->
        let hand_type =
          match hand_without_joker with
          | [ a; b; c; d ] when a = b && b = c && c = d -> FiveOfAKind
          | _ when max_count counts = 3 -> FourOfAKind
          | _ when max_count counts = 2 && is_two_pairs counts -> FullHouse
          | _ when max_count counts = 2 -> ThreeOfAKind
          | _ -> OnePair
        in
        { hand_type; hand; bid }
    | 3 ->
        let hand_type =
          match hand_without_joker with
          | [ a; b; c ] when a = b && b = c -> FiveOfAKind
          | _ when has_one_pair counts -> FourOfAKind
          | _ -> ThreeOfAKind
        in
        { hand_type; hand; bid }
    | 2 ->
        let hand_type =
          match hand_without_joker with
          | [ a; b ] when a = b -> FiveOfAKind
          | _ -> FourOfAKind
        in
        { hand_type; hand; bid }
    | _ -> { hand_type = FiveOfAKind; hand; bid }
  else
    let hand_type =
      match hand with
      | [ a; b; c; d; e ] when a = b && b = c && c = d && d = e -> FiveOfAKind
      | _ when max_count counts = 4 -> FourOfAKind
      | _ when max_count counts = 3 && has_one_pair counts -> FullHouse
      | _ when max_count counts = 3 -> ThreeOfAKind
      | _ when is_two_pairs counts -> TwoPairs
      | _ when max_count counts = 2 -> OnePair
      | _ -> HighCard
    in
    { hand_type; hand; bid }
